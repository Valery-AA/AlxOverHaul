...
class Alx_OT_Armature_BoneChainOnSelection(bpy.types.Operator):
    ...
    def execute(self, context: bpy.types.Context):
        if ( context.mode == "EDIT_MESH" ) and ( context.edit_object.type == "MESH" ):
            ...
            self.hair_strip_mesh = self.hair_strip_object.data
        
            self.hair_strip_mesh = context.edit_object.data
            self.hair_strip_bmesh = bmesh.from_edit_mesh(self.hair_strip_mesh).copy()
            ...
            if ( AlxRetiriveObjectModifier(self.hair_strip_object, "SUBSURF") is not None ):
                self.report({"INFO"}, message="[subdivision][modifier] | [present], [topology][source] | [base mesh]")

            edge_selection = [edge.index for edge in self.hair_strip_bmesh.edges if (edge.select == True)]
            ...
            edge_selection_validity = ( len(edge_selection_check) > 0 ) and all(edge_selection_check)
                        

            if (edge_selection_validity == False):
                self.report({"ERROR"}, "Selection is invalid - no more than two connected edge selected per vertex")
                return {"CANCELLED"}

            if (edge_selection_validity == True):
                chain_ends = [vert.index for edge_index in edge_selection for vert in self.hair_strip_bmesh.edges[edge_index].verts if (len([0 for edge in vert.link_edges if (edge.select == True)]) == 1)]
                ...
                for i in range(0, len(ordered_vert_chain)-1):
                    head = self.hair_strip_object.matrix_world @ self.hair_strip_bmesh.verts[ordered_vert_chain[i]].co
                    tail = self.hair_strip_object.matrix_world @ self.hair_strip_bmesh.verts[ordered_vert_chain[i+1]].co
                    vertex_normal = self.hair_strip_bmesh.verts[ordered_vert_chain[i]].normal

                    ordered_bone_position_map.update( { f"bone.v_index_{ordered_vert_chain[i]}" : [head, tail, vertex_normal] } )


                i=0
                while i < len(ordered_vert_chain)-1:
                    if ( self.hair_strip_object.vertex_groups.get( f"bone.v_index_{ordered_vert_chain[i]}" ) is None ):
                        self.hair_strip_object.vertex_groups.new( name=f"bone.v_index_{ordered_vert_chain[i]}" )
                    i+=1
                ...
                if (self.user_armature_name != ""):
                    ...
                    for i in range(0, len(ordered_vert_chain)):
                        if ( i < len(ordered_vert_chain)-1 ):
                            bone_vertex_group : bpy.types.VertexGroup = self.hair_strip_object.vertex_groups.get( f"bone.v_index_{ordered_vert_chain[i]}" )

                            if (bone_vertex_group is not None):
                                bone_edge = [v1_edge for v1_edge in self.hair_strip_bmesh.verts[ordered_vert_chain[i]].link_edges if ( v1_edge.index in [v2_edge.index for v2_edge in self.hair_strip_bmesh.verts[ordered_vert_chain[i+1]].link_edges] )][0] 
                                bone_vertex_group.add(index= [vert.index for vert in bone_edge.verts], weight=1, type="REPLACE")
                                ...
                                if (bone_edge is not None):
                                    ...
                                    i_rec = 0
                                    while (i_rec < 100):
                                        queue_edge = queue_edges.pop(0) if (len(queue_edges) > 0) else queue_edge
                                        
                                        vertex_blacklist = [*[vert.index for vert in queue_edge.verts], *[vert.index for vert in queue_edge.verts]]
                                        last_edge = []
                                        if (len(last_edge) >=2):
                                            break
                                                ...
                                                for f_edge in q_face.edges:
                                                    for f_vert in f_edge.verts:
                                                        ...
                                                        if (f_edge not in processed_edges):
                                                            bone_vertex_group.add(index= [vert.index for vert in f_edge.verts], weight=1, type="REPLACE")
                                                            last_edge.append(0)
                    ...
                    while i < len(ordered_vert_chain):
                        bone = armature_data.edit_bones.new(f"bone.v_index_{ordered_vert_chain[i]}") if armature_data.edit_bones.get(f"bone.v_index_{ordered_vert_chain[i]}") is None else armature_data.edit_bones.get(f"bone.v_index_{ordered_vert_chain[i]}")


                        if (bone is not None):
                            if (f"bone.v_index_{ordered_vert_chain[i]}" in ordered_bone_position_map.keys()):

                                head_vert = ordered_bone_position_map[f"bone.v_index_{ordered_vert_chain[i]}"][0]
                                bone.head.xyz = head_vert

                                tail_vert = ordered_bone_position_map[f"bone.v_index_{ordered_vert_chain[i]}"][1]
                                bone.tail.xyz = tail_vert

                                bone.parent = armature_data.edit_bones.get(f"bone.v_index_{ordered_vert_chain[i-1]}")
                                bone.use_connect = True
                                
                                bone.align_roll(vector=ordered_bone_position_map[f"bone.v_index_{ordered_vert_chain[i]}"][2])
